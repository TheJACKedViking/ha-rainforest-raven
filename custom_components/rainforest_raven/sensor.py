...
PRICE_SENSOR = RAVEnSensorEntityDescription(
...
async def async_setup_entry(
    ...
    for meter_mac_addr in entry.data[CONF_MAC]:
        ...
        entities.append(
            RAVEnPriceSensor(coordinator, PRICE_SENSOR, meter_mac_addr)
        )
...
class RAVEnPriceSensor(RAVEnMeterSensor):
    """Rainforest RAVEn Price Sensor with dynamic unit of measurement."""

    _cached_unit: str | None = None

    @property
    def native_unit_of_measurement(self) -> str | None:
        """Return the unit of measurement, derived from currency when available.

        Once a currency is discovered, the unit is cached to prevent HA
        long-term statistics resets when data temporarily disappears.
        """
        currency = self._data.get("currency")
        if currency is not None:
            currency_str = currency.value if hasattr(currency, "value") else str(currency)
            self._cached_unit = f"{currency_str}/{UnitOfEnergy.KILO_WATT_HOUR}"
