    ...
    async def _async_update_data(self) -> dict[str, Any]:
        for attempt in range(2):
            try:
                ...
                timeout = TIMEOUT_DEVICE_DATA if attempt == 0 else TIMEOUT_DEVICE_DATA // 2
                async with asyncio.timeout(timeout):
                    return await _get_all_data(
                        device, self.config_entry.data[CONF_MAC]
                    )
            except RAVEnConnectionError as err:
                if attempt == 0:
                    _LOGGER.debug(
                        "RAVEn connection error on attempt 1, reconnecting: %s", err
                    )
                    await self._cleanup_device()
                    continue
                await self._cleanup_device()
                ...
                if attempt == 0:
                    _LOGGER.debug(
                        "RAVEn timeout on attempt 1, resynchronizing and retrying"
                    )
                    try:
                        async with asyncio.timeout(TIMEOUT_DEVICE_CONNECT):
                            await device.synchronize()
                    except (TimeoutError, Exception):
                        await self._cleanup_device()
                        raise UpdateFailed(
                            "RAVEn device failed to resynchronize after timeout"
                        ) from err
                    continue
                await self._cleanup_device()
                raise UpdateFailed(
                    "RAVEn device timed out communicating with meter - "
                    "check USB connection and device status"
                ) from err
        raise UpdateFailed("Unexpected retry loop exit")

    async def _cleanup_device(self) -> None:
        device, self._raven_device = self._raven_device, None
        if device is not None:
            try:
                async with asyncio.timeout(TIMEOUT_DEVICE_CLEANUP):
                    await device.close()
            except Exception:  # Best-effort cleanup must not raise
                _LOGGER.warning("Error closing RAVEn device, forcing abort")
                try:
                    await device.abort()
                except Exception:
                    _LOGGER.debug("Abort also failed during cleanup", exc_info=True)
    ...
    async def _get_device(self) -> RAVEnSerialDevice:
        ...
        try:
            ...
            try:
                ...
                _LOGGER.debug(
                    "Failed to abort device during error recovery", exc_info=True
                )
            raise
